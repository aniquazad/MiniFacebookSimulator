 ...
 * uses the division method in hashing to store each person in the hash table;
 ...
 */
public class HashWithDiv 
{
	private FBLinkedList[] hashTable; //the hash table which stores people
	...
	public HashWithDiv()
	{
		hashTable = new FBLinkedList[LENGTH_OF_TABLE]; //makes hash table of a certain number of slot
		for(int i = 0; i < LENGTH_OF_TABLE; i++)
		{
			hashTable[i] = new FBLinkedList();//initializes the hash table
		}
	...
	/*
	 * This method is the division method used to determine which slot
	 * in the hash table the Person will be added. It adds up the ASCII
	 * value of each letter of the person's name and mods the total values 
	 ...
	 */
	public int divHashFxn(Person p)
	{
		String name = p.getName(); //gets the name of Person p
		int totalAscii = 0; //the total ASCII value of the name
		for(int i = 0; i < name.length(); i++)
		{
			totalAscii = (int)name.charAt(i); //gets the ASCII value of each char and adds it to to totalASCII
		}
		int htIndex = totalAscii % LENGTH_OF_TABLE; //calculates the index of Person p
		return htIndex;
	}
	...
	public void addTempFriends() 
	{
		...
		Person p1 = chainedHashSearch(hashTable, "Emily");
		Person p2 = chainedHashSearch(hashTable, "Noah");
		Person p3 = chainedHashSearch(hashTable, "Daniel");
		Person p4 = chainedHashSearch(hashTable, "Madison");
		Person p5 = chainedHashSearch(hashTable, "Scarlett");
		Person p6 = chainedHashSearch(hashTable, "Grace");
	 ...
	 */
	public void chainedHashInsert(FBLinkedList[] hashTable, Person p)
	{
		int htIndex = divHashFxn(p); //gets the index
		hashTable[htIndex].insert(p);//inserts p at the index
	}
	 ...
	 */
	public void chainedHashDelete(FBLinkedList[] hashTable, Person p)
	{
		int htKey = divHashFxn(p); //gets the index
		hashTable[htKey].delete(p); //deletes p from the index
	}
	 ...
	 */
	public Person chainedHashSearch(FBLinkedList[] hashTable, String name)
	{
		Person temp = new Person(name); //creates a temp Person of name
		int htKey = divHashFxn(temp); //gets the index
		Person found = hashTable[htKey].search(name); //searches instance of name at the index
	...
	public void option1()//create person
	{
		...
		chainedHashInsert(hashTable, p); //adds the Person to the hash table
		System.out.println();
		return;
	}
	...
	public void option2()//make a friend
	{
		...
		Person person1 = chainedHashSearch(hashTable, p1);//searches for Person of the name p1
		...
		Person person2 = chainedHashSearch(hashTable, p2);//searches for Person of the name p2
	...
	public void option4()//search a person's friend list
	{
		...
		Person found = chainedHashSearch(hashTable, p1);//searches for Person of the name p1
	...
	public void options()
	{
		...
		System.out.println("0. Multiplication method or quit");
		...
		{
			System.out.println("Would you like to try the multiplication method?");
			System.out.println("Press 8 for yes, 9 for no");
			int choice2 = in.nextInt();
			if(choice2 == 8)//have the option of seeing the same program with the multiplication method
			{
				HashWithMult hwm = new HashWithMult();
				hwm.start();
				return;
			}
			else//or terminating
	...
	/*
	 * This method is the main method which runs HashWithDiv and prints the header.
	 */
	public static void main(String[] args)
	{
		HashWithDiv hwd = new HashWithDiv();
		System.out.println("****************************************");
		System.out.println("WELCOME TO MICRO-FACEBOOK!");
		System.out.println("****************************************");
		System.out.println();
		hwd.add50People();
		hwd.addTempFriends();
